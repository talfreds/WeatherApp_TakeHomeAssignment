...
export default class UserService {
  async getUsersPublic() {
    const users = await UserModel.find({}).select("id _id name createdAt");
    return users;
  }
  async registerUser(input: Partial<User>) {
    try {
  ...
  async getSelf({ req, res, deserializeUser }: Context) {
    try {
  ...
  async refreshAccessToken({ req, res }: Context) {
    try {
      ...
      console.log(
        "🚀 ~ file: user.service.ts ~ line 138 ~ UserService ~ refreshAccessToken ~ decoded",
        decoded
      );
      if (!decoded) {
